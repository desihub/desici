#  RT171b.py  MLL UCB SSL 27 July 2019
#  Testing vacuum planet, KPNO zenith, and KPNO ZA=60 refractions.
# Goal is to validate refraction technique in RT171 and RT171a
# ... in particular their convoluted coordinate changers.  
# ADC actions should not much affect diameters: 2 microns in X, 5 microns in Y. 
# These are NOT changes in ngood.
#
#  Like RT171.py with just five.
#
#
...
def prepMatrixRefractedRay(M, coef, uv):
    global Rarray
    ...
    for iray in range(1, Nrays+1): 
        ...
        newUVW = rayUVW2bentUVW(M, coef, uvw)                    # getAtmosphereDeg(elev)
...
def getHexagonalNumbers(nrings, rmax):
    ...
    droll = 8.  # all rings rolled by 3 degrees
...
def rayUVW2bentUVW(Mtel, coef, rayUVW):
    # Matrix user
    ...
    lift = getAtmosphereDeg(coef, azel[1])               # get the refraction lift angle
...
#------------------- COEF------ZA----- ADC1------ADC2-----
tasks = np.array([[   0.0,    0.0,     0.0,     0.0],   # Airmass 0
                  [  0.00022, 0.0,     0.0,     0.0],   # Airmass 1
                  [  0.00022, 0.0,     90.,    -90.],   # Airmass 1
                  [  0.00022, 45.0,    0.0,     0.0],   # Airmass 1.41
                  [  0.00022, 45.0,    90.,    -90.],   # Airmass 1.41
                  [  0.00022, 60.0,    0.0,     0.0],   # Airmass 2
                  [  0.00022, 60.0,    90.,    -90.]])  # Airmass 2
...
outfilename = noext + '_Nstars_'+str(nspots) + '.txt'
...
# print('     COEF     ZA   ADC1  ADC2    RU        RV      Ngood    Xave      Yave       Xrms      Yrms')
print('         COEF     ZA   ADC1  ADC2      diamX      diamY')
...
with open(outfilename, 'w') as outfile:
    for t in tasks:
        ...
        cpoints = list()
        modifyADC(t[2], t[3])
        CenterAZEL = np.array([0., 90.-t[1]])
        Mtel = getSky2TelMatrix(CenterAZEL)
        for iuv in range(len(uvpairs)):
            uv = uvpairs[iuv]
            prepMatrixRefractedRay(Mtel, coef, uv)
            ru = Rarray[1,0,RU]
            rv = Rarray[1,0,RV]
            ngood, xave, yave, xrms, yrms = runCentroid()
            result = '{:10.6f}{:6.1f}{:6.1f},{:6.1f}{:10.6f}{:10.6f}{:6d}{:10.3f}{:10.3f}{:10.3f}{:10.3f}'  \
                .format(t[0],t[1],t[2],t[3],ru,rv, ngood, xave,yave,xrms,yrms)
            # print(result)
            outfile.writelines(result + '\n')  # yes plural even for one line
            if iuv == IXMAX: xmax = xave
            if iuv == IYMAX: ymax = yave
            if iuv == IXMIN: xmin = xave
            if iuv == IYMIN: ymin = yave
        diamx = xmax - xmin
        diamy = ymax - ymin
        print('  {:12.6f}{:6.1f}{:6.1f}{:6.1f}{:12.3f}{:12.3f}'.format(coef,t[1],t[2],t[3],diamx,diamy))
        
            
            

""" RESULTS...
RT171b calling prepMatrixRefractedRay(coef..) where coef=0 means vacuum planet
         COEF     ZA   ADC1  ADC2      diamX      diamY
      0.000000   0.0   0.0   0.0     280.166     280.166
      0.000220   0.0   0.0   0.0     280.103     280.103
      0.000220   0.0  90.0 -90.0     280.104     280.108
      0.000220  45.0   0.0   0.0     280.104     280.043
      0.000220  45.0  90.0 -90.0     280.104     280.042
      0.000220  60.0   0.0   0.0     280.106     279.923
      0.000220  60.0  90.0 -90.0     280.104     279.917


    COEF      ZA   ADC1  ADC2    RU        RV      Ngood    Xave      Yave       Xrms      Yrms
  0.000000   0.0   0.0,   0.0  0.000000  0.000000   408     0.007     0.000     0.007     0.007
  0.000000   0.0   0.0,   0.0  0.010000 -0.000000   363   140.091    -0.000     0.013     0.009
  0.000000   0.0   0.0,   0.0  0.000000  0.010000   364     0.007   140.083     0.009     0.012
  0.000000   0.0   0.0,   0.0 -0.010000  0.000000   363  -140.075    -0.000     0.012     0.009
  0.000000   0.0   0.0,   0.0 -0.000000 -0.010000   364     0.007  -140.083     0.009     0.012
  0.000220   0.0   0.0,   0.0  0.000000  0.000000   408     0.007     0.000     0.007     0.007
  0.000220   0.0   0.0,   0.0  0.009998 -0.000000   363   140.060    -0.000     0.013     0.009
  0.000220   0.0   0.0,   0.0  0.000000  0.009998   364     0.007   140.052     0.009     0.012
  0.000220   0.0   0.0,   0.0 -0.009998  0.000000   363  -140.044    -0.000     0.012     0.009
  0.000220   0.0   0.0,   0.0 -0.000000 -0.009998   364     0.007  -140.052     0.009     0.012
  0.000220   0.0  90.0, -90.0  0.000000  0.000000   408     0.000     4.973     0.008     0.008
  0.000220   0.0  90.0, -90.0  0.009998 -0.000000   363   140.052     5.012     0.012     0.009
  0.000220   0.0  90.0, -90.0  0.000000  0.009998   364     0.000   145.097     0.009     0.012
  0.000220   0.0  90.0, -90.0 -0.009998  0.000000   363  -140.052     5.012     0.012     0.009
  0.000220   0.0  90.0, -90.0 -0.000000 -0.009998   364     0.000  -135.011     0.010     0.013
  0.000220  60.0   0.0,   0.0 -0.000000 -0.000381   408     0.007    -5.309     0.007     0.007
  0.000220  60.0   0.0,   0.0  0.009998 -0.000381   364   140.061    -5.338     0.013     0.009
  0.000220  60.0   0.0,   0.0  0.000000  0.009610   364     0.007   134.564     0.009     0.012
  0.000220  60.0   0.0,   0.0 -0.009998 -0.000381   364  -140.045    -5.338     0.012     0.009
  0.000220  60.0   0.0,   0.0 -0.000000 -0.010372   364     0.007  -145.359     0.009     0.012
  0.000220  60.0  90.0, -90.0 -0.000000 -0.000381   408     0.000    -0.336     0.008     0.008
  0.000220  60.0  90.0, -90.0  0.009998 -0.000381   364   140.052    -0.326     0.012     0.009
  0.000220  60.0  90.0, -90.0  0.000000  0.009610   364     0.000   139.604     0.009     0.012
  0.000220  60.0  90.0, -90.0 -0.009998 -0.000381   364  -140.052    -0.326     0.012     0.009
  0.000220  60.0  90.0, -90.0 -0.000000 -0.010372   364     0.000  -140.314     0.010     0.013

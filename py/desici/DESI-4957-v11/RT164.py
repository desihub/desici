#  RT164.py  MLL UCB SSL 3 July 2019
#
...
def prepRefractedRayGroup(za, u, v):
    global Rarray
    ...
    for iray in range(1, Nrays+1): 
        ...
        uvw = np.array([u,v,w])
        rayAZEL = rayUVW2azel(centerAZEL, uvw)
        rayZA = 90.-rayAZEL[1]
        ...
        newUVW = azel2rayUVW(centerAZEL, newAZEL)
...
def azel2rayUVW(CenterAZEL, targetAZEL):
    ...
    return uvw                            # send toray tracer for optical distortions
...
myRayFileName = 'DESI-9.RAY.CSV'  # Seven wavels 408*7=2856 rays; first group is "d" line 0.5878 um
...
tasks = np.array([[   0.0,    0.0,    0.0],       # AtmosZA, ADC1, ADC2 angles for zenith work
                  [   0.0,  36.74, -36.74],
                  [  45.0,    0.0,    0.0],
                  [  45.0, 36.74, -36.74]])   
...
outfilename = noext + '_Nrings_' + str(NRINGS) + '_Rmax_' + str(RMAX) + '.txt'
...
with open(outfilename, 'w') as outfile:
    for t in tasks:
        ...
        modifyADC(t[1], t[2])
        for uv in uvpairs:
            prepRefractedRayGroup(t[0], uv[0], uv[1])
            ru = Rarray[1,0,RU]
            rv = Rarray[1,0,RV]
            ngood, xave, yave, xrms, yrms = runCentroid()
...
""" result for NRINGS=1, RMAX=25, DESI-9.RAY.CSV using 408 rays monochromatic wavel="d"

   ZA   ADC1  ADC2    RU        RV      Ngood    Xave      Yave       Xrms      Yrms
   
   0.0   0.0   0.0  0.000000  0.000000   408     0.007     0.000     0.007     0.007
   0.0   0.0   0.0  0.021646  0.012497   328   312.136   180.207     0.010     0.012
   0.0   0.0   0.0  0.000000  0.024995   328     0.008   360.410     0.013     0.008
   0.0   0.0   0.0 -0.021646  0.012497   328  -312.112   180.202     0.010     0.012
   0.0   0.0   0.0 -0.021646 -0.012497   328  -312.112  -180.202     0.010     0.012
   0.0   0.0   0.0 -0.000000 -0.024995   328     0.008  -360.410     0.013     0.008
   0.0   0.0   0.0  0.021646 -0.012497   328   312.136  -180.207     0.010     0.012
   
   0.0  36.7 -36.7  0.000000  0.000000   408     0.006     2.975     0.007     0.008
   0.0  36.7 -36.7  0.021646  0.012497   328   312.185   183.372     0.010     0.013
   0.0  36.7 -36.7  0.000000  0.024995   328     0.007   363.661     0.014     0.007
   0.0  36.7 -36.7 -0.021646  0.012497   328  -312.164   183.364     0.009     0.012
   0.0  36.7 -36.7 -0.021646 -0.012497   328  -312.066  -177.044     0.010     0.012
   0.0  36.7 -36.7 -0.000000 -0.024995   328     0.005  -357.164     0.011     0.009
   0.0  36.7 -36.7  0.021646 -0.012497   328   312.084  -177.044     0.010     0.011

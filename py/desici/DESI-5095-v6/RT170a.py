#  RT170a.py  MLL UCB SSL July 2019
#  Uses the as-measured ADC angle tasks instead of my demo grid of ADC tasks
#  Bt but is this wise?? Perhaps better to show the theo regular grid and the data points?
#
#
#  Like 169 but larger field: NRINGS=8, UVMAX=28mrad, so XYMAX=407mm.
#  Uses DESI-9 files. 
#
# General Writeup:  DESI-DOC 2745v12
#   ... but this versioin lacks Zernike surfaces and Sag Grid surfaces 
#
#  Understanding and quantifying ADC distortions:
#  Exploring eight diagnostic distances between five C.I. field locations
#  Using 5 target directions spanning 27mrad radius
#  Using 408 pupil rays on axis (fewer off axis) monochromatic "d" line
#  Stepping through 24 ADC settings
#  Evaluating spot centroid locations and sizes on focal plane
#  Writing RT167_Nfields_5_Rmax_27.txt file:  24*5 = 120 records, 9 columns 
#
#
#  Adopting double retro format  DESI-9.OPT  DESI-9.RAY  DESI-9.MED
...
#
# OUTLINE:
#   Library imports:    line 66...
#   Macro definitions:   line 80...
#   List definitions:    lines 230...
#   Table parsing tools:  lines 300..
#   Math helpers:         lines 530...  
#   Surface generators:  lines 650...
#   Slope generators:   lines 750..
#   Interceptors:      lines 900...
#   Validator:           lines 1040...
#   Redirectors:            lines 1100...
#   Coordinate changers:      lines 1200...
#   Text output formatters:     lines 1300...
#   Input file unpackers:    lines 1500...
#   AutoRay support:      lines 1900...
#   Ray tracing:           lines 2000...
#   Output graphics:               lines 2200.....
#   Main program with INPUT FILE NAMES:  lines 2350...
#  
...
def prepTableRayGroup(wantListing):
    # print('Starting prepTableRayGroup()...')
    # Sets up Rarray[] using Raystarts[] data
    for iray in range(1, Nrays+1): 
        #----Fill Rarray[0] with using table RayStarts----------
        Rarray[iray, 0, RWAVE]  = Raystarts[iray, RWAVE]           
        ...
        if wantListing:
            print("prepOneRayGroup(): X,Y,Z,U,V,W = {:12.6f}{:12.6f}{:12.6f}{:12.6f}{:12.6f}{:12.6f}".format(X,Y,Z,U,V,W))

def prepDeviatedRayGroup(du,dv):
    # allows steering of a single-direction group of table rays, for multiple spot diagrams
    # du, dv are towards +x, +y using double retro convention
    # Affects Rarray[] but not Raystarts[]
    # print('Starting prepDeviatedRayGroup()...')
    for iray in range(1, Nrays+1): 
        #----Fill Rarray[0] with using table RayStarts----------
        Rarray[iray, 0, RWAVE]  = Raystarts[iray, RWAVE]           
        X = Rarray[iray, 0, RX] = Raystarts[iray, RX]
        Y = Rarray[iray, 0, RY] = Raystarts[iray, RY]
        Z = Rarray[iray, 0, RZ] = Raystarts[iray, RZ]
        U = Rarray[iray, 0, RU] = Raystarts[iray, RU] + du  # modify u
        V = Rarray[iray, 0, RV] = Raystarts[iray, RV] + dv  # modify v
        W = Rarray[iray, 0, RW] = Raystarts[iray, RW]
        W = Rarray[iray, 0, RW] = fixup(U,V,W)
        
def prepForcedRayGroup(u, v):
    # allows steering of all table rays, for multiple spots
    # overwrites the table's "u" and "v" fields
    # print('Starting prepForcedRayGroup()...')
    ...
    for iray in range(1, Nrays+1): 
        ...
        Y = Rarray[iray, 0, RY] = Raystarts[iray, RY]
        Z = Rarray[iray, 0, RZ] = Raystarts[iray, RZ]
        U = Rarray[iray, 0, RU] = u                        # force u
        V = Rarray[iray, 0, RV] = v                        # force v
        W = Rarray[iray, 0, RW] = Raystarts[iray, RW]
        W = Rarray[iray, 0, RW] = fixup(U,V,W)
        
    

def getAtmosphericRefraction(wavelum, zadeg):
    # Sellmeier approximation, Peck & Reeder JOSA v.62 (1972)
    # Results in radians:
    # 0.365000   45.000000  0.000226138006
    # 0.588000   45.000000  0.000219961045
    # 1.014000   45.000000  0.000217544915
    A = 57918.27
    B = 238.0185
    C = 1679.09
    D = 57.362
    P = 790  # mbar,  typical KPNO barometer
    T = 283  # kelvin, typical KPNO thermometer
    wm2 = wavelum**(-2)
    sell = A/(B-wm2) + C/(D-wm2)   # microradians
    return 1E-6*(P*288)/(T*1013) * sell * tand(zadeg)


        
def prepRefractedRayGroup(u, v):
    global Rarray
    ...
    for iray in range(1, Nrays+1): 
        ...
        w = Rarray[iray, 0, RW] = Raystarts[iray, RW]
        w = Rarray[iray, 0, RW] = fixup(u,v,w)    
        # next evaluate the added refraction in elevation
        uvw = np.array([u,v,w])
        rayAZEL = rayUVW2azel(Mtel, uvw)
        rayZA = 90.-rayAZEL[1]
         
        if iray == -200:
            printVector('prepRefracted() is given CenterAZEL = ', centerAZEL)
            printVector('prepRefracted() has deviated u,v,w  = ', uvw)
            printVector('prepRefracted() is given rayAZEL    =   ', rayAZEL)  
            print('prepRefracted is using rayZA = {:12.6f}'.format(rayZA))          
               
        dELdeg = DEG * getAtmosphericRefraction(wavel, rayZA)
        newAZEL = rayAZEL + np.array([0., dELdeg])
        newUVW = azel2rayUVW(Mtel, newAZEL)
        nu = Rarray[iray, 0, RU] = newUVW[0]
        nv = Rarray[iray, 0, RV] = newUVW[1]
        nw = Rarray[iray, 0, RW] = np.sqrt(1. - nu*nu - nv*nv)


def runTableRayGroup():   
    # run all table rays to Nsurfs and produce global output lists
    # BE SURE to have run one of the prepRays() routines, to grab Raystarts. 
    global xpoints, ypoints, cpoints,  xAVE, yAVE, xRMS, yRMS, rRMS
    # print 'Starting runTableRayGroup()...'
    xpoints = list()   # empty the host list.
    ypoints = list()   # empty the host list.
    cpoints = list()   # empty the host list.
    xDEVs   = list()   # empty the host list.
    yDEVs   = list()   # empty the host list.
    ngood = 0
    for iray in range(1, Nrays+1):                     # base=1
        howfar  = runOneRay(iray)
        if howfar == Nsurfs:                           # this is a "good" ray
            ngood += 1
            x = Rarray[iray, Nsurfs, RX] 
            y = Rarray[iray, Nsurfs, RY] 
            xpoints.append(x)                          # lab frame list, mm
            ypoints.append(y)                          # lab frame list, mm
            color = getColorFromName(RwaveNames[iray])
            cpoints.append(color)                      # base=0 
            
    #---now evaluate all the statistics for this group.  Units are mm-----        
    xAVE = np.average(xpoints)            # labframe global output, mm
    yAVE = np.average(ypoints)            # labframe global outout, mm
    xdevs = xpoints - xAVE                # local list of x deviations, mm
    ydevs = ypoints - yAVE                # local list of y deviations, mm
    xRMS = np.std(xdevs)                  # xRMS global output, mm
    yRMS = np.std(ydevs)                  # yRMS global output, mm 
    rRMS = np.sqrt(xRMS**2 + yRMS**2)     # rRMS global output, mm
    print('runTableRayGroup(): ngood,xAVE,yAVE= {:6d}{:9.3f}{:9.3f}'.format(ngood,xAVE,yAVE))
    return ngood
...
def getXYZ(lonlat):  # Convert spherical angles into its xyz triplet
    return np.array([cosd(lonlat[0])*cosd(lonlat[1]), 
                     sind(lonlat[0])*cosd(lonlat[1]), 
                     sind(lonlat[1])])    
                     
def getLONLAT(xyz): # Convert xyz triplet into its spherical angles
...
def getSky2TelMatrix(FieldCenterAZEL): 
    # Evaluates the 3x3 unitary transform from sky directions to telescope ray {u,v,w}
...
myOptFileName = 'DESI-9.OPT.CSV'  # like DESI-9.OPT but without any FP Diameter
myRayFileName = 'DESI-9.RAY.CSV'  # 408 rays, monochromatic "d" line, fill on-axis pupil
...
#---If we need a Media table, load and verify it-------
...
origtasks = np.array([              # ADC1, ADC2 angles at zenith
                  [   0.0,    0.0], 
                  [  30.0,    0.0],
                  [  60.0,    0.0],                  
                  [  90.0,    0.0],
...
measuredtasks = np.array([           # Google sheet "adc_in_program"
                  [   0.0,    0.0], 
                  [  29.0,    0.0],  
                  [  60.0,    0.0],                  
                  [  89.0,  359.0],
                  ...
                  [ 359.0,  180.0],
                  [ 359.0,  210.0],                  
                  [ 359.0,  240.0],    # 250was a typo
                  [ 359.0,  270.0],
                  [ 359.0,  300.0],
                  [ 359.0,  330.0],
                  [ 359.0,    0.0]])               
                  
NRINGS = 8  # gives 217 sky locations
UVMAX  = 28 # milliradians


# Sky data (millimeters, CS5) with ADC=0 on focal plane from PlotGuideStars2.py
# These went into GetSkyUV.py and get their sky {u,v} coordinates
nfp = np.array([   -2.247,  394.255])
wfp = np.array([  397.055,   -8.787])
cfp = np.array([   -7.597,   -2.989])
efp = np.array([ -396.284,   -2.934])
sfp = np.array([    3.898, -396.513])
allfp = np.array([nfp, wfp, cfp, efp, sfp])


noext = PROGNAME.rsplit('.',1)[0]    
outfilename = noext + '_Nrings_' + str(NRINGS) + '_Rmax_' + str(UVMAX) + '.txt'

# Reverse ZhaoBurge fit to the five observed focal plane mm data above:
# GetSkyUV.py results with ADC=0 zenith observations
# Actual data here although inverse ZB evaluated to convert fp {xy} to sky{uv}
uvpairs = np.array([[ -0.000155,   0.027169 ], 
                    [  0.027346,  -0.000605 ],
                    [ -0.000546,  -0.000215 ],
                    [ -0.027299,  -0.000202 ],
                    [  0.000268,  -0.027313 ]])           
...
print('Now writing predicted raytraced fp {xy} from sky {uv} stars in C.I. database...')
                            
outfilename = 'RT170a_Nstars_5_SkyXYinferredUV.txt'
print('  ADC1  ADC2    RU        RV      Ngood    Xave      Yave       Xrms      Yrms')


with open(outfilename, 'w') as outfile:
    for t in measuredtasks:
        xpoints = list()
        ypoints = list()
        cpoints = list()
        modifyADC(t[0], t[1])
        for uv in uvpairs:
            prepForcedRayGroup(uv[0], uv[1])
            ru = Rarray[1,0,RU]
            rv = Rarray[1,0,RV]
            ngood, xave, yave, xrms, yrms = runCentroid()
            result = '{:6.1f}{:6.1f}{:10.6f}{:10.6f}{:6d}{:10.3f}{:10.3f}{:10.3f}{:10.3f}'  \
                .format(t[0],t[1],ru,rv, ngood, xave,yave,xrms,yrms)
            print(result)
            outfile.writelines(result + '\n')  # yes plural even for one line
            
""" results: 25 frames, bad pointings have been avoided so not 27 frames
  ADC1  ADC2    RU        RV      Ngood    Xave      Yave       Xrms      Yrms
   0.0   0.0 -0.000155  0.027169   327    -2.242   394.254     0.012     0.013
   0.0   0.0  0.027346 -0.000605   326   397.059    -8.784     0.012     0.011
   0.0   0.0 -0.000546 -0.000215   408    -7.600    -2.995     0.007     0.007
   0.0   0.0 -0.027299 -0.000202   326  -396.284    -2.932     0.012     0.012
   0.0   0.0  0.000268 -0.027313   324     3.898  -396.520     0.011     0.012
   0.0   0.0 -0.000155  0.027169   327    -2.242   394.254     0.012     0.013
   0.0   0.0  0.027346 -0.000605   326   397.059    -8.784     0.012     0.011
   0.0   0.0 -0.000546 -0.000215   408    -7.600    -2.995     0.007     0.007
   0.0   0.0 -0.027299 -0.000202   326  -396.284    -2.932     0.012     0.012
   ...
   0.0   0.0 -0.027299 -0.000202   326  -396.284    -2.932     0.012     0.012
   0.0   0.0  0.000268 -0.027313   324     3.898  -396.520     0.011     0.012
  29.0   0.0 -0.000155  0.027169   327    -2.574   395.596     0.012     0.013
  29.0   0.0  0.027346 -0.000605   326   396.710    -7.497     0.013     0.011
  29.0   0.0 -0.000546 -0.000215   408    -7.912    -1.788     0.007     0.007
  29.0   0.0 -0.027299 -0.000202   326  -396.631    -1.648     0.012     0.012
  29.0   0.0  0.000268 -0.027313   324     3.565  -395.177     0.012     0.012
  60.0   0.0 -0.000155  0.027169   327    -3.567   396.654     0.012     0.012
  60.0   0.0  0.027346 -0.000605   326   395.671    -6.485     0.013     0.011
  60.0   0.0 -0.000546 -0.000215   408    -8.845    -0.839     0.008     0.007
  60.0   0.0 -0.027299 -0.000202   326  -397.667    -0.638     0.012     0.012
  60.0   0.0  0.000268 -0.027313   324     2.572  -394.123     0.012     0.013
  89.0 359.0 -0.000155  0.027169   327    -4.847   397.073     0.012     0.012
  89.0 359.0  0.027346 -0.000605   326   394.335    -6.085     0.014     0.011
  89.0 359.0 -0.000546 -0.000215   408   -10.046    -0.463     0.008     0.007
  89.0 359.0 -0.027299 -0.000202   326  -399.004    -0.235     0.012     0.011
  89.0 359.0  0.000268 -0.027313   324     1.295  -393.705     0.012     0.013
 119.0   0.0 -0.000155  0.027169   327    -6.180   396.679     0.012     0.012
 119.0   0.0  0.027346 -0.000605   326   392.946    -6.463     0.014     0.011
 119.0   0.0 -0.000546 -0.000215   408   -11.297    -0.818     0.008     0.008
 119.0   0.0 -0.027299 -0.000202   326  -400.398    -0.613     0.012     0.011
 119.0   0.0  0.000268 -0.027313   324    -0.036  -394.099     0.012     0.013
 150.0   0.0 -0.000155  0.027169   327    -7.189   395.641     0.012     0.013
 150.0   0.0  0.027346 -0.000605   326   391.894    -7.455     0.015     0.011
 150.0   0.0 -0.000546 -0.000215   408   -12.247    -1.750     0.008     0.008
 150.0   0.0 -0.027299 -0.000202   326  -401.458    -1.606     0.012     0.012
 150.0   0.0  0.000268 -0.027313   324    -1.047  -395.135     0.012     0.012
 179.0   0.0 -0.000155  0.027169   327    -7.541   394.305     0.012     0.013
 179.0   0.0  0.027346 -0.000605   326   391.526    -8.733     0.015     0.011
 179.0   0.0 -0.000546 -0.000215   408   -12.580    -2.952     0.008     0.008
 179.0   0.0 -0.027299 -0.000202   326  -401.830    -2.888     0.012     0.012
 179.0   0.0  0.000268 -0.027313   324    -1.404  -396.470     0.012     0.012
 210.0   0.0 -0.000155  0.027169   327    -7.183   392.874     0.012     0.014
 210.0   0.0  0.027346 -0.000605   326   391.897   -10.104     0.015     0.011
 210.0   0.0 -0.000546 -0.000215   408   -12.247    -4.241     0.008     0.008
 210.0   0.0 -0.027299 -0.000202   326  -401.459    -4.262     0.012     0.012
 210.0   0.0  0.000268 -0.027313   324    -1.052  -397.905     0.012     0.012
 240.0 359.0 -0.000155  0.027169   327    -6.212   391.911     0.012     0.015
 240.0 359.0  0.027346 -0.000605   326   392.909   -11.029     0.014     0.011
 240.0 359.0 -0.000546 -0.000215   408   -11.335    -5.109     0.008     0.007
 240.0 359.0 -0.027299 -0.000202   326  -400.442    -5.186     0.012     0.011
 240.0 359.0  0.000268 -0.027313   324    -0.081  -398.872     0.012     0.012
 269.0   0.0 -0.000155  0.027169   327    -4.934   391.493     0.012     0.015
 269.0   0.0  0.027346 -0.000605   326   394.243   -11.433     0.014     0.011
 269.0   0.0 -0.000546 -0.000215   408   -10.134    -5.485     0.008     0.007
 269.0   0.0 -0.027299 -0.000202   326  -399.103    -5.585     0.012     0.011
 269.0   0.0  0.000268 -0.027313   324     1.199  -399.292     0.012     0.011
 299.0   0.0 -0.000155  0.027169   327    -3.605   391.838     0.012     0.015
 299.0   0.0  0.027346 -0.000605   326   395.634   -11.103     0.013     0.011
 299.0   0.0 -0.000546 -0.000215   408    -8.883    -5.173     0.008     0.007
 299.0   0.0 -0.027299 -0.000202   326  -397.711    -5.251     0.012     0.012
 299.0   0.0  0.000268 -0.027313   324     2.533  -398.945     0.012     0.011
 329.0   0.0 -0.000155  0.027169   327    -2.620   392.830     0.012     0.014
 329.0   0.0  0.027346 -0.000605   326   396.665   -10.151     0.013     0.011
 329.0   0.0 -0.000546 -0.000215   408    -7.956    -4.278     0.007     0.007
 329.0   0.0 -0.027299 -0.000202   326  -396.680    -4.297     0.012     0.012
 329.0   0.0  0.000268 -0.027313   324     3.521  -397.947     0.011     0.012
 359.0   0.0 -0.000155  0.027169   327    -2.242   394.206     0.012     0.013
 359.0   0.0  0.027346 -0.000605   326   397.059    -8.831     0.012     0.011
 359.0   0.0 -0.000546 -0.000215   408    -7.600    -3.039     0.007     0.007
 359.0   0.0 -0.027299 -0.000202   326  -396.285    -2.979     0.012     0.012
 359.0   0.0  0.000268 -0.027313   324     3.898  -396.568     0.011     0.012
   0.0  29.0 -0.000155  0.027169   327    -1.910   392.920     0.011     0.014
   0.0  29.0  0.027346 -0.000605   326   397.407   -10.068     0.012     0.011
   0.0  29.0 -0.000546 -0.000215   408    -7.289    -4.199     0.007     0.007
   0.0  29.0 -0.027299 -0.000202   326  -395.940    -4.213     0.012     0.012
   0.0  29.0  0.000268 -0.027313   324     4.231  -397.856     0.012     0.011
   0.0  60.0 -0.000155  0.027169   327    -0.921   391.870     0.011     0.015
   0.0  60.0  0.027346 -0.000605   326   398.442   -11.078     0.012     0.011
   0.0  60.0 -0.000546 -0.000215   408    -6.359    -5.146     0.007     0.008
   0.0  60.0 -0.027299 -0.000202   326  -394.909    -5.219     0.013     0.012
   0.0  60.0  0.000268 -0.027313   324     5.222  -398.906     0.011     0.011
   0.0  90.0 -0.000155  0.027169   327     0.400   391.501     0.011     0.015
   0.0  90.0  0.027346 -0.000605   326   399.822   -11.434     0.012     0.012
   0.0  90.0 -0.000546 -0.000215   408    -5.117    -5.478     0.008     0.008
   0.0  90.0 -0.027299 -0.000202   326  -393.534    -5.573     0.014     0.012
   0.0  90.0  0.000268 -0.027313   324     6.544  -399.276     0.011     0.011
 359.0 120.0 -0.000155  0.027169   327     1.721   391.822     0.011     0.015
 359.0 120.0  0.027346 -0.000605   326   401.202   -11.127     0.012     0.012
 359.0 120.0 -0.000546 -0.000215   408    -3.876    -5.189     0.008     0.008
 359.0 120.0 -0.027299 -0.000202   326  -392.158    -5.265     0.015     0.012
 359.0 120.0  0.000268 -0.027313   324     7.865  -398.957     0.012     0.011
 359.0 150.0 -0.000155  0.027169   327     2.688   392.830     0.012     0.014
 359.0 150.0  0.027346 -0.000605   326   402.211   -10.159     0.012     0.011
 359.0 150.0 -0.000546 -0.000215   408    -2.967    -4.280     0.008     0.008
 359.0 150.0 -0.027299 -0.000202   326  -391.150    -4.298     0.016     0.011
 359.0 150.0  0.000268 -0.027313   324     8.833  -397.949     0.012     0.012
 359.0 180.0 -0.000155  0.027169   327     3.042   394.206     0.012     0.013
 359.0 180.0  0.027346 -0.000605   326   402.579    -8.836     0.012     0.011
 359.0 180.0 -0.000546 -0.000215   408    -2.635    -3.039     0.008     0.008
 359.0 180.0 -0.027299 -0.000202   326  -390.781    -2.977     0.016     0.011
 359.0 180.0  0.000268 -0.027313   324     9.186  -396.572     0.012     0.012
 359.0 210.0 -0.000155  0.027169   327     2.688   395.582     0.012     0.012
 359.0 210.0  0.027346 -0.000605   326   402.208    -7.512     0.012     0.011
 359.0 210.0 -0.000546 -0.000215   408    -2.967    -1.797     0.008     0.008
 359.0 210.0 -0.027299 -0.000202   326  -391.150    -1.656     0.016     0.011
 359.0 210.0  0.000268 -0.027313   324     8.831  -395.193     0.011     0.013
 359.0 240.0 -0.000155  0.027169   327     1.721   396.589     0.012     0.012
 359.0 240.0  0.027346 -0.000605   326   401.197    -6.542     0.012     0.012
 359.0 240.0 -0.000546 -0.000215   408    -3.876    -0.889     0.008     0.008
 359.0 240.0 -0.027299 -0.000202   326  -392.157    -0.690     0.015     0.011
 359.0 240.0  0.000268 -0.027313   324     7.863  -394.184     0.011     0.013
 359.0 270.0 -0.000155  0.027169   327     0.400   396.958     0.012     0.012
 359.0 270.0  0.027346 -0.000605   326   399.817    -6.186     0.012     0.012
 359.0 270.0 -0.000546 -0.000215   408    -5.117    -0.556     0.008     0.008
 359.0 270.0 -0.027299 -0.000202   326  -393.533    -0.336     0.014     0.012
 359.0 270.0  0.000268 -0.027313   324     6.540  -393.814     0.011     0.014
 359.0 300.0 -0.000155  0.027169   327    -0.921   396.589     0.012     0.012
 359.0 300.0  0.027346 -0.000605   326   398.437    -6.540     0.012     0.011
 359.0 300.0 -0.000546 -0.000215   408    -6.359    -0.889     0.007     0.008
 359.0 300.0 -0.027299 -0.000202   326  -394.908    -0.691     0.013     0.012
 359.0 300.0  0.000268 -0.027313   324     5.218  -394.182     0.011     0.013
 359.0 330.0 -0.000155  0.027169   327    -1.888   395.582     0.012     0.012
 359.0 330.0  0.027346 -0.000605   326   397.428    -7.507     0.012     0.011
 359.0 330.0 -0.000546 -0.000215   408    -7.268    -1.797     0.007     0.007
 359.0 330.0 -0.027299 -0.000202   326  -395.916    -1.658     0.012     0.012
 359.0 330.0  0.000268 -0.027313   324     4.251  -395.190     0.011     0.013
 359.0   0.0 -0.000155  0.027169   327    -2.242   394.206     0.012     0.013
 359.0   0.0  0.027346 -0.000605   326   397.059    -8.831     0.012     0.011
 359.0   0.0 -0.000546 -0.000215   408    -7.600    -3.039     0.007     0.007
 359.0   0.0 -0.027299 -0.000202   326  -396.285    -2.979     0.012     0.012

...
def anyFunc(uv, *parms): # uv = 1D array input concatenate(u[],v[])
...
outfilename = 'ZBF_Nrings_' + str(NRINGS) + '_Rmax_' + str(RMAX) + '.txt'

with open(outfilename, 'w') as outfile:
    for field in range(Nfields):
        adc12 = alldata[field*Neach:(field+1)*Neach, 0:2] # rows,cols
        uv    = alldata[field*Neach:(field+1)*Neach, 2:4] # rows,cols
        xy    = alldata[field*Neach:(field+1)*Neach, 5:7] # rows,cols
        uv *= 1000./RMAX  # for unit circle
        xy *= 1./390.     # for unit circle
        # print('adc12 =')
        # print(adc12)
        # print('uv = ')
        # print(uv)
        ...
        #---check using identity initial parms: does xyfit = uv?------

        # print('args array = ')
        # print(args)
        print('args shape = ')
        print(args.shape)                      # should be 217*2=434
        xyfit = anyFunc(args, *pinitial)       # initial parms = 0: no deviation
        # print('initial xyfit = ')
        # print(xyfit)                           # should equal args.
        print('xyfit.shape = ')
        print(xyfit.shape)                     # should be...434

        popt, covar = curve_fit(anyFunc, args, goals, pinitial)
        #  note: curve_fit() calls its given func(args, *parms)
        print('\npopt: \n', popt)

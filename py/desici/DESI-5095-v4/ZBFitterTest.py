...
#
#  Here, trying FPMAX = 386.3684 millimeters to get P[2]=0.
#  Result for ADC=00, forward, 11 ZB coefs:
#      Xtrans      Ytrans      Magnif    UpDownUp  RightLeftRight  InOut      OutInOut     Roll   LeftRightCurl  UpDownCurl  RollAntiRoll
#     0.000024    0.000002    0.000000   -0.000001    0.000012    0.018954    0.000188   -0.000000   -0.000005    0.000001    0.000000
#  Result for ADC=00, reverse, 11 ZB coefs:
#    -0.000023   -0.000002    0.000348    0.000001   -0.000010   -0.018293    0.000453    0.000000    0.000004   -0.000001    0.000000  
#
#  Odd that although P[2]forward=0, P[2]reverse is not zero.
#
# Assumes that RT166.py has already run a complete monochromatic trace,
# with ~ 400 rays per pupil, and ~200 field locations with image centroids,
...
infilename = 'RT166_Nrings_8_Rmax_27.txt'  # 5208 records = 24 x 217
...
FPMAX     = 386.3684  # 390    # millimeters


# first get the forward (sky to FP) fits to the 24 ADC tasks

outfilename = 'ZBF_Test_fwd_Nrings_' + str(NRINGS) + '_Rmax_' + str(RMAX) + '.txt'

with open(outfilename, 'w') as outfile:
    for field in range(Nadcs):
        adc12 = np.copy(alldata[field*Neach:(field+1)*Neach, 0:2])    # rows,cols
        uv    = np.copy(alldata[field*Neach:(field+1)*Neach, 2:4])    # rows,cols
        xy    = np.copy(alldata[field*Neach:(field+1)*Neach, 5:7])    # rows,cols
        uv *= 1000./RMAX   # for unit circle scaling
        xy *= 1./FPMAX     # for unit circle scaling
        print()
        ...
        #  note: curve_fit() calls its given func(args, *parms)
        print('popt[2]: {:12.6f}'.format(popt[2]))
...
# then get the reverse (FP to sky) fits to the 24 ADC tasks

outfilename = 'ZBF_Test_rev_Nrings_' + str(NRINGS) + '_Rmax_' + str(RMAX) + '.txt'
